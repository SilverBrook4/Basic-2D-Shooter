...
class Engine {
    private:
        ...
        /// @brief player to be rendered.
        /// @details Initialized in initShapes()
        unique_ptr<Player> player;

        /// @breif a vector of enemies to be renderes
        /// @details initialized in initShapes()
        vector<unique_ptr<Rect>> enemies;

        /// @breif a vector of projectiles shot by player
        /// @details a new projectile is spawned when left mouse pressed
        vector<unique_ptr<Projectile>> playerProjectiles;

        /// @breif the x and y component of the mouse
        double MouseX, MouseY;

        /// @breif the slope of the players aim
        vec2 aimSlope;
    ...
    public:
        ...
        /// @breif Reutens true if collision detected
        /// @param projectile the projectile being shot
        /// @param box the rectangle colision is being checked againts
        /// @return true collision detected
        /// @return false collision NOT detected
        bool detectProjectileCollision(unique_ptr<Projectile>& projectile, unique_ptr<Rect>& box);
